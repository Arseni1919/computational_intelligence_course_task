...
def SA(iteration, history, cities, facilities, cells, dist):
    ...
    if new_util > curr_best_util:
        ...
        # exponenta = np.exp(-1 * (curr_best_util - new_util) / (iteration + 1))
        exponenta = np.exp(-1 / (iteration + 1))
        if random.random() > exponenta:
            change_cells_from_to(cells, copy_cells)
