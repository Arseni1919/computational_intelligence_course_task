...
def local_search(iteration, history, cities, facilities, cells, allocations, dist):
    ...
    if iteration == 0:
        ...
        best_utility = calc_utility(allocations, cells)
        best_next_allocations = copy_allocations(allocations)
        ...
        best_next_location = get_cell_of_facility(cells, facility_to_move)

        # choose randomly 10 possible new positions for the chosen facility
        new_random_locations = get_next_random_positions(cells)

        # calculate the utility of moving the facility, and choose the best possible location to move to
        for new_random_location in new_random_locations:
            ...
            if best_utility < next_positions_utility:
                best_next_allocations = copy_allocations(next_allocations)
                best_next_location = Cell(new_random_location.cell_size, new_random_location.surf_center, new_random_location.ord_number, new_random_location.cost)
                best_utility = next_positions_utility

        change_facility_location_in_cells(cells, facility_to_move, current_facility_location, best_next_location)
        copy_allocations_from_to(best_next_allocations, allocations)
